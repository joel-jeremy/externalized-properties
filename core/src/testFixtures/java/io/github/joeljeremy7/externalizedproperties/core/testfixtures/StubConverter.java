...
public class StubConverter<T> implements Converter<T> {
  ...
  public StubConverter(Predicate<Class<?>> canConvertTo, ConvertDelegate<T> convert) {
  ...
  public ConversionResult<T> convert(
    ...
    ConversionResult<T> result = convert.convert(context, valueToConvert, targetType);
