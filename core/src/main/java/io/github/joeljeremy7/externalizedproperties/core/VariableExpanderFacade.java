...
/**
 * This annotation is used to indicate that the annotated method will serve as a facade to 
 * the underlying registered Externalized Properties variable expander. Variable expander 
 * facades are expected to be with a specific signature. The first parameter must be the 
 * value to expand ({@code String}) and the return type must also be a {@code String}.
 * 
 * <p>Examples:</p>
 * 
 * <blockquote><pre> 
