 ...
 * underlying registered Externalized Properties resolvers.
 *
 * <p>Resolver facades are expected to be with a specific signature. The first parameter must be the
 * property name ({@code String}). The return type can be any type. Conversion will automatically be
 * done.
 *
 * <p>Optionally, it is also possible to specify a second parameter: the target type for the
 * property which may be represented by the following classes: {@link TypeReference}, {@link Class},
 * or {@link Type}. The method's return type can be a generic type variable e.g. {@code <T>} or any
 * type as long as the target type is assignable to the it.
 *
 * <p>If a property is converted to the target type and the converted value is not assignable to the
