...
public class RootConverter implements Converter<Object> {
  ...
  public RootConverter(Collection<Converter<?>> converters) {
    this.convertersByTargetType =
        new ConvertersByTargetType(requireNonNull(converters, "converters"));
  }
  ...
  public ConversionResult<Object> convert(
    ...
    try {
      for (Converter<Object> converter : converters) {
        ConversionResult<Object> result = converter.convert(context, valueToConvert, targetType);
      ...
      throw new ConversionException(
          String.format(
              ...
              rawTargetType.getName(), ExternalizedProperties.class.getSimpleName()));
    } catch (ConversionException cex) {
      throw cex;
    } catch (RuntimeException ex) {
      throw new ConversionException(
          String.format(
              "Exception occurred while converting value to target type: %s. " + "Value: %s",
              rawTargetType.getName(), valueToConvert),
          ex);
    }
  }
